...
User UserService::Register(std::string _name, std::string _password)
{
	...
	system((std::string("mkdir ./UserFile/") + user.name).c_str());

	return user;
}

std::optional<std::string> UserService::Login(std::string _name, std::string _password)
{
...
std::optional<User> UserService::GetUser(std::string _token)
{
	auto result = this->table.Query()
